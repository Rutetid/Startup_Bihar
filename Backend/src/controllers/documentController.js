...
const uploadDocuments = async (req, res) => {
	try {
		...
		const newDocumentData = {
			...
			coFounderNames: req.body.coFounderNames
				? req.body.coFounderNames.trim()
				: null, // Use as a single string
			coFounderAadharNumbers: req.body.coFounderAadharNumbers
				? req.body.coFounderAadharNumbers.trim()
				: null, // Use as a single string
			sector: req.body.sector,
			businessConcept: req.body.businessConcept,
			mobileNumbers: req.body.mobileNumbers
				? req.body.mobileNumbers.trim()
				: null, // Use as a single string
...
const updateDocumentStatus = async (req, res) => {
	...
	try {
		...
		res
			.status(500)
			.json({ error: "Failed to update document status and comment" });
	}
};
/*const updateDocumentStatus = async (req, res) => {
  const { id } = req.params;
